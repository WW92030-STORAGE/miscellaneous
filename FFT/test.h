...
void test1() {
    ...
    vector<FFT::Complex> v({cf(1, 0), cf(2, -1), cf(0, -1), cf(-1, 2)});
    
    auto res = FFT::DFT(v);
    auto res2 = FFT::FFT(v);
    std::cout << FFT::disp(res) << "\n";
    std::cout << FFT::disp(res2) << "\n";
    
    int N = (1<<15);
    v = vector<FFT::Complex>(N);
    for (int i = 0; i < v.size(); i++) v[i] = FFT::Complex(randrad() * 32, randrad() * 32);
    res = DFT(v);
...
void functest() {
    ...
    int N = 128;
    int M = 5;
    std::vector<FFT::Complex> inputs(N);
    for (int i = 0; i < N; i++) inputs[i] = FFT::Complex(0.1 * i * i - 0.005 * i * i * i + 0.000063 * i * i * i * i, 0);
    for (int i = 0; i < N; i++) inputs[i] = FFT::Complex((i % (M<<1) < M) ? 1 : -1, 0);
    // for (int i = 0; i < N; i++) inputs[i] = Complex(cos(0.125 * M_PI * i), sin(0.125 * M_PI * i));

    auto freq = FFT::FFT(inputs);
    ...
    std::vector<FFT::Complex> solution(N);
    for (int i = 0; i < N; i++) {
        for (int k = 0; k < N - 2; k++) {
